...
#define toSizeT(t)      ((int)((t + SIZE_T_1) / SIZE_T)) * SIZE_T  // TODO
//
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO
      for(MemBlock<T>* ptr = _activeBlock; ptr != 0;){
         MemBlock<T>* next = ptr -> getNextBlock();
         if(next){
            delete ptr;
            _activeBlock = ptr = next;
         }
         else
            ptr = next;
      }
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      while(list -> getArrSize() != n){
         if(list -> getNextList() == 0){
            MemRecycleList<T>* newList = new MemRecycleList<T>(n);
            list -> setNextList(newList);
   ...
   T* getMem(size_t t) {
      ...
      //    Note: rn is the array size
            ...
            if(_activeBlock -> getRemainSize() >= S)
               rn = getArraySize(_activeBlock -> getRemainSize());
            if(_activeBlock -> getRemainSize() == S)
               rn = 0;
   ...
   size_t getNumBlocks() const {
      // TODO
      size_t count = 1;
      for(MemBlock<T>* it = _activeBlock; it -> getNextBlock() != 0; it = it -> getNextBlock()){
         count++;
      }
      return count;
   }
